...
/**
 * {@link Functions} implementation for jython language.
 * 
 * @author JunHo Yoon
 * @since 1.0
 */
public class JythonFunctions extends Functions {
	/**
	 * Constructor.
	 * 
	 * @param urlMap
	 ...
	 *            genetationOpts
	 */
	public JythonFunctions(Map<String, BaseURIType> urlMap, Map<String, CommonHeadersType> headerMap,
	...
	public String generatePathString(RelativeURIType relativeURIType) {
		...
		for (TokenReferenceType tokenReference : tokenReferenceArray) {
			...
			sourceBuilder.append(getTokenMap().get(tokenReference.getTokenId()).getName()).append("=%s");
			assignmentBuilder.append("self.").append(tokenReference.getTokenId());
			if (index == length - 1) {
				assignmentBuilder.append(")");
			} else {
				sourceBuilder.append("&");
		...
		sourceBuilder.append("\"");
		sourceBuilder.append(assignmentBuilder);
	...
	public String generateHeaderParameter(HeadersType headers) {
		...
		for (HeaderType headerType : headerArray) {
			...
			headerBuilder.append("\t\t\t\t\tNVPair(").append(escapeQuote(headerType.getName())).append(", ")
	...
	public String generateTokenReference(TokenReferenceType token) {
		...
		tokenBuilder.append("self.").append(token.getTokenId()).append(" = ");
	...
	/**
	 * Convert a bytes array to jython recognized string.
	 * 
	 * 
	 * @param bytes
		...
		if (body.getForm() != null) {
			if (body.getForm().getFormFieldArray().length != 0) {
				for (FormFieldType formFieldType : body.getForm().getFormFieldArray()) {
					postBuilder.append("\t\t\t\t\tNVPair(").append(escapeQuote(formFieldType.getName())).append(", ")
									.append(escapeQuote(formFieldType.getValue())).append("),\n");
				}
			}
	...
	public String escapeQuote(String value) {
		...
		return XSLTHelper.quoteForPython(value);
	}
